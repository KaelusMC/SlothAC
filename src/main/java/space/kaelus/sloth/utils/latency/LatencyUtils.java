...
public class LatencyUtils implements ILatencyUtils {
    ...
    public LatencyUtils(SlothPlayer player) {
        this.player = player;
    }
        ...
        synchronized (transactionMap) {
            ...
            for (Runnable runnable : tasksToRun) {
                try {
                    ...
                    SlothAC.getInstance().getLogger().severe("An error occurred when running transactions for player: " + player.getUser().getName());
                    e.printStackTrace();
